...
<?php
$has_disclaimer = !empty($footer_data['disclaimer']);
$has_logos = !empty($comp_logos);
$has_social_icons = !empty($footer_data['facebook']) || !empty($footer_data['twitter']) || !empty($footer_data['youtube']) || !empty($footer_data['instagram']) || !empty($footer_data['linkedin']);

$disclaimer_classes = 'text-sm m-0 leading-[1.6]';
if ($has_disclaimer && (!$has_logos || !$has_social_icons)) {
    $disclaimer_classes .= ' lg:flex-1 text-center mx-auto';
} elseif ($has_disclaimer) {
    $disclaimer_classes .= ' lg:flex-1';
}

$logos_wrap_classes = 'flex items-center justify-center gap-6 flex-wrap pt-[30px] lg:pt-0';
if ($has_disclaimer) {
    $logos_wrap_classes .= ' lg:justify-end lg:ml-6';
} else {
    $logos_wrap_classes .= ' lg:justify-center lg:mx-auto';
}
$footer_layout = in_array(($footer_data['layout'] ?? 'new'), ['original', 'new'], true) ? $footer_data['layout'] : 'new';
?>
<?php if ($footer_layout === 'original') : ?>
    <footer class="[&_a]:text-white [&_a]:font-bold [&_a]:no-underline [&_a]:transition-all [&_a]:duration-300 [&_a:hover]:opacity-70 [&_ul]:!list-none [&_ul]:!ml-0 [&_ul]:!pr-0 [&_ul]:!mb-0 [&_.bottom-footer-column-logo_img]:!max-w-[100px]">
        ...
        ?>
        <div class="top-footer bg-topFooter bg-primary pt-8 pb-12">
            ...
            <div class="container px-8">
                <div class="grid grid-cols-12 md:gap-x-8">
                    <div class="col-span-12 md:col-span-6 lg:col-span-4">
                        <?php if (!empty($footer_data['old_footer_legal_left'])) { ?>
                            <div class="[&_p:first-child]:pt-2">
                                <?php echo wp_kses_post($footer_data['old_footer_legal_left']); ?>
                            </div>
                        <?php } ?>
                    </div>
                    <div class="col-span-12 md:col-span-6 lg:col-span-5">
                        <?php if (!empty($footer_data['disclaimer'])) : ?>
                            <div class="text-sm leading-[1.7] text-white/85"><?php echo wp_kses_post($footer_data['disclaimer']); ?></div>
                        <?php endif; ?>
                    </div>
                    <div class="col-span-12 lg:col-span-3">
                        <?php if (!empty($comp_logos)) { ?>
                            <div class="bottom-footer-column-logos pt-4 text-center items-center flex gap-x-8 justify-start flex-wrap">
                                <?php foreach ($comp_logos as $logo) : ?>
                                    <a href="<?php echo esc_url($logo['url']); ?>" <?php if ($logo['url'] !== '#') echo 'target="_blank" rel="noreferrer"'; ?>>
            ...
            <div class="container px-8">
                <div class="grid grid-cols-12 xl:gap-x-8">
                    <div class="col-span-12 xl:col-span-4">
                        <div class="py-2 text-white/80">&copy; <?php echo esc_html(gmdate('Y')); ?> <?php bloginfo('name'); ?>. All Rights Reserved.</div>
                    </div>
                    <div class="col-span-12 xl:col-span-5">
                        <?php if (!empty($legal_links)) { ?>
                            <nav class="flex flex-wrap items-center gap-1 py-2">
                                <?php foreach ($legal_links as $i => $link) : ?>
                                    <?php if ($i > 0) : ?><span class="mx-[.4rem] text-white/30">|</span><?php endif; ?>
                                    <a href="<?php echo esc_url($link['url']); ?>" class="!font-normal text-white/80 hover:text-white">
                    ...
                    <div class="col-span-12 xl:col-span-3 mb-4 pt-2">
                        <?php if (!empty($footer_data['old_footer_legal_right'])) { ?>
                            <?php
                            $site_created_by = wp_kses_post($footer_data['old_footer_legal_right']);
                            $site_created_by = preg_replace(
                                '/<p(\b[^>]*)>/i',
                                '<p$1 style="margin-top:0!important;padding-top:0!important;margin-block-start:0!important;padding-block-start:0!important;">',
                                $site_created_by,
                                1
                            );
                            echo $site_created_by;
                            ?>
                        <?php } ?>
                    </div>
                </div>
        ...
        <div class="container px-8 pb-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between mb-6">
                <?php if ($has_disclaimer) : ?>
                    <div class="<?php echo esc_attr($disclaimer_classes); ?>">
                        <?php echo wp_kses_post($footer_data['disclaimer']); ?>
                    </div>
                <?php endif; ?>
                <div class="<?php echo esc_attr($logos_wrap_classes); ?>">
                    <?php if ($has_logos) : ?>
                        <?php foreach ($comp_logos as $logo) : ?>
