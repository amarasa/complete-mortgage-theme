...
<div class="zip-code-form-or-button-widget bg-grey pt-8 pb-6 px-10 mb-5">
    ...
    <?php } ?>
    <?php if ($subheadline) { ?>
        <h5 class="text-xl text-[#515151]"><?php echo esc_html($subheadline); ?></h5>
    <?php } ?>
    <div class="mt-5">
        <?php if ($gravity_form_or_button == true) { ?>
            <?php if ($zip_code_form_or_button_widget_button) { ?>
                <a class="button w-full" href="<?php echo esc_url($zip_code_form_or_button_widget_button['url']); ?>" <?php echo cms_link_attributes($zip_code_form_or_button_widget_button['target'] ?: '_self'); ?>><?php echo esc_html($zip_code_form_or_button_widget_button['title']); ?></a>
            <?php } ?>
        <?php } else { ?>
            <?php echo do_shortcode('[gravityform id=' . absint($gravity_form) . ' ajax="true" description="false" title="false"]'); ?>
        <?php } ?>
    </div>
    <?php if ($after_form_or_button_text) { ?>
        <div class="text-base">
            <?php echo wp_kses_post($after_form_or_button_text); ?>
        </div>
    <?php } ?>
</div>
