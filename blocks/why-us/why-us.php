...
<section class="why-us bg-gray-100 pt-10 pb-6 <?php echo esc_attr($classes); ?>" <?php echo $id; ?> data-block-name="<?php echo esc_attr($acfKey); ?>">
                        ...
                        <div class="grid grid-cols-12 lg:gap-x-8">
                            <?php foreach ($partial_row_list as $item): ?>
                                <div class="col-span-12 md:col-span-6">
                                    ...
                                    <div>
                                        <h3 class="!text-base font-bold text-secondary tracking-[1px] uppercase !mb-0"><?php echo esc_html($item['headline']); ?></h3>
                                        <div class="text-gray-700"><?php echo $item['content']; ?></div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
            ...
            <?php else: ?>
                ...
                <?php if (!empty($full_row_list)): ?>
                    <?php foreach ($full_row_list as $item): ?>
                        <div class="col-span-12 md:col-span-6 lg:col-span-4 gap-x-8">
                            <div class="w-12 h-12 items-center justify-center bg-primary rounded-full text-2xl mb-2 relative">
                                <i class="<?php echo esc_attr($item['icon']); ?> text-white top-1/2 left-1/2 absolute -translate-y-1/2 -translate-x-1/2" aria-hidden="true"></i>
                            </div>
                            <div>
                                <h3 class="!text-base text-secondary tracking-[1px] font-bold uppercase !mb-0"><?php echo esc_html($item['headline']); ?></h3>
                                <div class="text-gray-700 wysiwyg"><?php echo $item['content']; ?></div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
