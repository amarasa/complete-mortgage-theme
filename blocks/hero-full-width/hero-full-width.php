...
<section class="hero-full-width relative w-full h-full overflow-hidden <?php echo $default_background_color; ?> <?php echo esc_attr($classes); ?>" <?php echo $id; ?> data-block-name="<?php echo esc_attr($acfKey); ?>">
    ...
    <div class="hero-full-width-content relative px-8 z-20 <?php echo $content_text_color; ?> mx-auto text-center container py-16 lg:pt-[130px] lg:pb-[105px]">
        ...
        <h1 class="<?php echo $content_text_color; ?>"><?php echo esc_html($headline); ?></h1>
        <div class="max-w-3xl mx-auto mb-5 <?php echo $content_text_color; ?>"><?php echo $introduction_text; ?></div>
            ...
            <div class="mt-4 <?php echo $button_count === 3 ? 'md:flex justify-center gap-x-8' : ''; ?>">
                <?php foreach ($buttons as $button): ?>
                    <?php if (!empty($button['button'])): ?>
                        <a class="button !no-underline !text-white !block md:!max-w-[350px] w-full mb-3 mx-auto <?php echo $button_bg_class; ?> <?php echo $button_count === 3 ? '!max-w-none w-auto px-6' : ''; ?>" href="<?php echo esc_url($button['button']['url']); ?>" <?php echo cms_link_attributes($button['button']['target'] ?: '_self'); ?>>
        ...
        <p><?php echo wp_kses_post(str_replace('<a ', '<a class="!no-underline ' . $content_text_color . ' hover:!opacity-60" ', $below_button_text)); ?></p>
    </div>
</section>