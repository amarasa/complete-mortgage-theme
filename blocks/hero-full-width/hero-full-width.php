...
$show_gradient_overlay = !empty($turn_on_overlay);
$top_gradient_color = !empty($top_gradient_overlay) ? $top_gradient_overlay : 'rgba(0,0,0,0.8)';
$bottom_gradient_color = !empty($bottom_gradient_overlay) ? $bottom_gradient_overlay : 'rgba(0,0,0,0.4)';
$overlay_style = sprintf(
    'background: linear-gradient(to bottom, %1$s 0%%, %2$s 100%%);',
    esc_attr($top_gradient_color),
    esc_attr($bottom_gradient_color)
);
?>
<?php
$hero_image_id = 0;
...
<section class="hero-full-width relative w-full h-full overflow-hidden <?php echo esc_attr($default_background_color); ?> <?php echo esc_attr($classes); ?>" <?php echo $id; ?> data-block-name="<?php echo esc_attr($acfKey); ?>">
    ...
    <?php if ($hero_image_id && $show_gradient_overlay) { ?>
        <div class="hero-full-width-overlay z-10 absolute h-full w-full" style="<?php echo esc_attr($overlay_style); ?>"></div>
    <?php } ?>
    <?php
