...
<section class="trusted-by cmt-block relative <?php echo esc_attr($classes); ?> <?php echo esc_attr($enable_bg_color ? 'bg-grey' : ''); ?> <?php echo esc_attr($bg_url ? 'max-h-[500px] overflow-hidden' : ''); ?>" <?php echo $id_attr; ?> data-block-name="<?php echo esc_attr($acfKey); ?>">
    <?php if ($bg_url) { ?>
                ...
                <div class="col-span-12 lg:col-span-8 lg:col-start-3">
                    ...
                    <?php else: ?>
                        <div class="logo-flex flex flex-wrap items-center justify-center gap-6">
                            <?php if (have_rows('logos')): while (have_rows('logos')): the_row(); ?>
                                    <div class="logo-card <?php echo esc_attr($enable_bg_color ? 'border border-primary bg-white' : ''); ?> bg-card bg-white rounded-lg p-6 shadow-sm border hover:shadow-md transition-all duration-300 flex items-center justify-center md:max-w-[175px] w-full">
                                        <?php
                                        $logo    = get_sub_field('logo');
                                        $logo_id = is_array($logo) ? ($logo['ID'] ?? 0) : (int)$logo;
                                        if ($logo_id) {
