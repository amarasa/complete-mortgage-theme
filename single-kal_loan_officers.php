...
<section class="lo-breadcrumb mt-10 container px-8 mb-4">
    <?php
    $link_generated = false; // Flag to indicate if a link has already been generated

    if (has_term('', 'branch_location')) {
        foreach (get_the_terms(get_the_ID(), 'branch_location') as $tax) {
            if ($link_generated) {
                break; // Break the loop if a link has already been generated
            }

            if (have_rows('branch_pages', 'option')) {
                while (have_rows('branch_pages', 'option')) {
                    the_row();
                    if (get_sub_field('branch_name') == $tax->name) {
                        $link_generated = true; // Set the flag to true
    ?>
                        <a href="<?php echo the_sub_field('branch_page'); ?>" class="text-sm font-semibold block"><span class="breadcrumb-arrow text-sm font-semibold relative">←</span> Back to our <?php echo __($tax->name); ?> Branch</a>
                <?php
                ...
                $link_generated = true; // Set the flag to true
                ?>
                <a class="text-sm font-semibold block" href="<?php the_permalink(58); ?>"><span class="breadcrumb-arrow text-sm font-semibold relative">←</span> Back to About Us</a>
            <?php
            }
        }
    } else {
        $fallback_link = get_field('fallback_back_to_about_us', 'option'); // Retrieve the array from the theme settings
        if ($fallback_link) {
            $link_url = $fallback_link['url'] ?? '#'; // Fallback URL is '#' if not set
            $link_title = $fallback_link['title'] ?? 'About Us'; // Default title if not set
            ?>
            <a class="text-sm font-semibold block" href="<?php echo esc_url($link_url); ?>"><span class="breadcrumb-arrow text-sm font-semibold relative">←</span> Back to <?php echo esc_html($link_title); ?></a>
        <?php
        } else {
        ?>
            <a class="text-sm font-semibold block" href="<?php the_permalink(58); ?>"><span class="breadcrumb-arrow text-sm font-semibold relative">←</span> Back to About Us</a>
