...
<main class="loan-officer-single">
<section class="lo-breadcrumb !py-0 mt-2 container px-8 mb-0">
    <?php $about_us_page = get_field('about_us_page', 'option'); ?>
    <nav aria-label="Breadcrumb" class="mb-3 text-sm font-semibold">
        <ol class="flex flex-wrap items-center gap-2 !mt-0 !mb-12 !ml-0 !p-0 !list-none text-primary">
            <li class="!mb-0"><a href="<?php echo esc_url(home_url('/')); ?>" class="!no-underline">Home</a></li>
            <?php if ($about_us_page) { ?>
                <li class="!mb-0" aria-hidden="true">/</li>
                <li class="!mb-0"><a href="<?php echo esc_url($about_us_page['url']); ?>" class="!no-underline"><?php echo esc_html($about_us_page['title'] ?? 'About Us'); ?></a></li>
            <?php } ?>
            <li class="!mb-0" aria-hidden="true">/</li>
            <li class="!mb-0 text-[#515151]" aria-current="page"><?php the_title(); ?></li>
        </ol>
    </nav>
    <?php if ($about_us_page) { ?>
        <a class="text-sm font-semibold block" href="<?php echo esc_url($about_us_page['url']); ?>"><span class="breadcrumb-arrow text-sm font-semibold relative">←</span> Back to About Us</a>
    <?php } ?>
</section>
<section class="container !py-0 px-8 mb-24">
            ...
            <div class="lo-meta mb-8">
                ...
                <?php if (get_field('nmls_number')) { ?>
                    ...
                    <div class="text-base uppercase tracking-wider inline-block">
                        NMLS #<?php echo esc_html(get_field('nmls_number')); ?>
                    </div>
                <?php } ?>
            </div>
        </div>
            ...
            <?php if (get_field('phone_number') || get_field('cell') || get_field('fax') || get_field('email_address') || get_field('facebook') || get_field('linkedin') || get_field('twitter')) { ?>
                ...
                <div class="mb-4 text-sm leading-6">
                    <?php if (get_field('phone_number')) { ?>
                        Phone: <a href="tel:<?php echo esc_attr(preg_replace('/[^0-9+]/', '', get_field('phone_number'))); ?>"><?php echo esc_html(get_field('phone_number')); ?></a><br>
                    <?php } ?>
                    <?php if (get_field('cell')) { ?>
                        Cell: <a href="tel:<?php echo esc_attr(preg_replace('/[^0-9+]/', '', get_field('cell'))); ?>"><?php echo esc_html(get_field('cell')); ?></a><br>
                    <?php } ?>
                    <?php if (get_field('fax')) { ?>
                        Fax: <?php echo esc_html(get_field('fax')); ?><br>
                    <?php } ?>
                    <?php if (get_field('email_address')) { ?>
                        <a href="mailto:<?php echo esc_attr(sanitize_email(get_field('email_address'))); ?>" class="text-charcoal-grey"><?php echo esc_html(get_field('email_address')); ?></a>
                    <?php } ?>
                </div>
                <div class="flex mb-5 items-center gap-x-4">
                    <?php if (get_field('facebook')) { ?>
                        <a href="<?php echo esc_url(get_field('facebook')); ?>" target="_blank" rel="noopener noreferrer">
                            <i class="fab fa-facebook"><span class="sr-only"><?php echo esc_html(get_the_title()); ?> on Facebook</span></i>
                        </a>
                    <?php } ?>
                    <?php if (get_field('linkedin')) { ?>
                        <a href="<?php echo esc_url(get_field('linkedin')); ?>" target="_blank" rel="noopener noreferrer">
                            <i class="fab fa-linkedin"><span class="sr-only"><?php echo esc_html(get_the_title()); ?> on LinkedIn</span></i>
                        </a>
                    <?php } ?>
                    <?php if (get_field('twitter')) { ?>
                        <a href="<?php echo esc_url(get_field('twitter')); ?>" target="_blank" rel="noopener noreferrer">
                            <i class="fab fa-x-twitter"><span class="sr-only"><?php echo esc_html(get_the_title()); ?> on Twitter</span></i>
                        </a>
                    <?php } ?>
                    <?php if (get_field('instagram')) { ?>
                        <a href="<?php echo esc_url(get_field('instagram')); ?>" target="_blank" rel="noopener noreferrer">
                            <i class="fab fa-instagram"><span class="sr-only"><?php echo esc_html(get_the_title()); ?> on Instagram</span></i>
                        </a>
                    <?php } ?>
                    <?php if (get_field('youtube')) { ?>
                        <a href="<?php echo esc_url(get_field('youtube')); ?>" target="_blank" rel="noopener noreferrer">
                            <i class="fab fa-youtube"><span class="sr-only"><?php echo esc_html(get_the_title()); ?> on YouTube</span></i>
                        </a>
                    <?php } ?>
                </div>
            <?php } ?>
            ...
            ?>
                <div class="lo-links">
                    <div class="items-center">
                        ...
                        <?php if ($cta_button) { ?>
                            <a id="apply<?php if (get_field('nmls_number')) { ?>-<?php echo esc_attr(get_field('nmls_number')); ?><?php } ?>" href="<?php echo esc_url($cta_button['url']); ?>" class="apply-button button text-base mr-16 mb-10" <?php echo cms_link_attributes($cta_button['target'] ?: '_self'); ?>><?php echo esc_html($cta_button['title']); ?></a>
                        <?php } ?>
                    ...
                    <div class="md:w-[250px] md:text-center">
                        <?php if (get_field('view_licensing_link')) { ?>
                            <a href="<?php echo esc_url(get_field('view_licensing_link')); ?>" class="text-base font-semibold mb-10 block" id="view-licensing<?php if (get_field('nmls_number')) { ?>-<?php echo esc_attr(get_field('nmls_number')); ?><?php } ?>">View Licensing</a>
                        <?php } ?>
                    </div>
                </div>
            <?php } else { ?>
                <div class="lo-links">
                    <div class="md:flex lg:flex xl:flex items-center">
                        ...
                        <?php } ?>
                        <?php if (get_field('view_licensing_link')) { ?>
                            <a href="<?php echo esc_url(get_field('view_licensing_link')); ?>" class="text-base font-semibold cursor-pointer mb-10 block" id="view-licensing<?php if (get_field('nmls_number')) { ?>-<?php echo esc_attr(get_field('nmls_number')); ?><?php } ?>">View Licensing</a>
                        <?php } ?>
                    </div>
                </div>
            <?php } ?>
